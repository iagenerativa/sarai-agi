...
def test_selector_respeta_override_de_calidad():
    selector = DynamicQuantizationSelector(min_ram_free_gb=1.0, force_quality_threshold=0.85)
...
def test_selector_reduce_cuando_prompt_corto():
    selector = DynamicQuantizationSelector(min_ram_free_gb=1.0)
...
def test_update_historial_incrementa_conteos():
    selector = DynamicQuantizationSelector()
